...
IMPORTANT — accuracy: if the product is a branded/packaged item and you are not certain of its exact ingredients, use web search to find the retailer's actual published ingredient list before scoring. Never guess ingredients for branded products when a search can confirm them. Base hazards on the real ingredient list. Be efficient: one well-chosen search is usually enough; do not search at all if the ingredients are already provided or the item is generic (e.g. a plain pan, a lemon).
...
After any research, respond with ONLY a JSON object as your final answer, no markdown fences, no preamble:
{
  ...
  "nutrition": null for non-food items, otherwise: {
    "per": "the basis, e.g. 'per pack' or 'per 100g' — prefer per pack/serving when known",
    "protein_g": number or null,
    "sugar_g": number or null,
    "calories_kcal": number or null,
    "nutrients": [up to 6 notable vitamins/minerals/nutrients actually present, each {"name": "e.g. Vitamin B12", "amount": "e.g. 1.2ug", "rda_percent": number percentage of adult daily recommended intake (UK NRV or US DV as appropriate for the region)}] — empty array if none notable,
    "artificial": [list of artificial/synthetic ingredients present: E-numbers with their name and purpose, artificial sweeteners, flavourings, colours, preservatives — e.g. "E471 mono/diglycerides (emulsifier)". Empty array if none]
  }
}

Use the real published nutrition figures (from web search or provided database info) — do not invent numbers. If a figure is unknown, use null rather than guessing.
...
export default async function handler(req, res) {
  ...
  // speed: if the product database already gave us the ingredients, skip web search entirely
  const hasDbIngredients = !!text && text.includes("ingredients:") && !text.includes("ingredients: not listed");
  const tools = hasDbIngredients ? undefined : [{ type: "web_search_20250305", name: "web_search", max_uses: 2 }];
